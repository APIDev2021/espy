/*
    ...
    * Modifications by Espy (https://espimyte.com/)
*/

// The values in this section are REQUIRED for the widget to work! Keep them in quotes!
const s_stylePath = '/style/comment.css';
const s_formId = '1FAIpQLSdJiWxVWQxCz3AulyxXsMImzfyzRN9IT1OEfZT6Twd4_toHxw';
const s_nameId = '1895151353';
const s_websiteId = '122651136';
const s_textId = '2016666567';
const s_pageId = '1004055054';
const s_replyId = '596011692';
const s_sheetId = '1v7Y7loHMuFRTyiABA_P8Wfs9ZDzhi76PAMvYPyB7qxc';
...
const s_widgetTitle = 'Ask me. Tell me. I\'m listening.';
...
const s_commentsTitle = 'Comments';
const s_noCommentsText = 'Nothing here yet.';
...
const v_mainHtml = `
    <div id="c_container">
        <h1 id="c_commentsTitle">${s_commentsTitle}</h1>
        <p class="c-info">${s_loadingText}</p>
    </div>
    <div id="c_inputDiv">
...
const v_formHtml = `
    ...
    <div id="c_textWrapper" class="c-inputWrapper">
        ...
        <textarea class="c-input c-textInput" name="entry.${s_textId}" id="entry.${s_textId}" rows="4" cols="50"  maxlength="${s_maxLength}" required placeholder="Your message here!"></textarea>
    </div>
...
function getComments() {
    ...
    retrievedSheet.then(result => {
        ...
        if (comments.length == 0 || Object.keys(comments[0]).length < 2) { // Once again, Google Sheets can be weird
            c_container.innerHTML = `<h1 id="c_commentsTitle">${s_commentsTitle} (${comments.length})</h1><p class="c-info">${s_noCommentsText}</p>`;
        } else {
...
function displayComments(comments) {
    ...
    a_commentDivs = [];
    c_container.innerHTML = `<h1 id="c_commentsTitle">${s_commentsTitle} (${comments.length})</h1>`;
    ...
    for (i = 0; i < comments.length; i++) {
        ...
        // Reply button
        replyButtonsContainer = document.createElement('div');
        ...
        replyButtonsContainer.appendChild(button);
        replyButtonsContainer.className = 'c-replyButtonsContainer';
        comment.appendChild(replyButtonsContainer);
    ...
    for (i = 0; i < replies.length; i++) {
        ...
        if (!document.getElementById(parentId + '-replies')) { 
            ...
            if (s_collapsedReplies) {container.style.display = 'block'} // Default to shown if collapsed
    ...
    if (s_collapsedReplies) {
        ...
        for (i = 0; i < containers.length; i++) {
            ...
            const replyButtonsContainer = parentDiv.getElementsByClassName('c-replyButtonsContainer')[0];

            // The button to expand replies
            const button = document.createElement('button');
            button.innerHTML = s_hideRepliesText + ` (${num})`;
            button.onclick = function (_event) {
                const expanded = expandReplies(this.parentElement.parentElement.id);
                if (!expanded) {
...
function createComment(data) {
    ...
    commentHeader.className = "c-header";
    ...
    if (url) {
        ...
        site.innerText = url.split('/')[2];
        site.href = url;
        site.className = 'c-site';
        commentHeader.appendChild(site);
    }
...
function convertTimestamp(timestamp) {
    ...
    return [offsetDate.toLocaleString().replace(',', ''), offsetDate.toLocaleDateString()];
}
// DST checker
function isDST(date) {
